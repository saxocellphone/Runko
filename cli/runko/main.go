...
func cmdChange(args []string) error {
	valid := map[string]bool{"create": true, "push": true, "requirements": true, "land": true, "approve": true, "list": true, "abandon": true, "rerun-check": true, "comment": true, "comments": true, "resolve": true, "request-review": true, "automerge": true}
	if len(args) < 1 || !valid[args[0]] {
		return usageError("usage: runko change create|push|requirements|land|approve|list|abandon|rerun-check|comment|comments|resolve|request-review|automerge ... (see docs/cli-contract.md)")
	}
	switch args[0] {
	case "create":
