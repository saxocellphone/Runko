...
var Commands = []Command{
	...
	{"runko", "project create --name <n> --type <t> [--lang l] [--no-template] [--owners a,b] [--json]", "create a project from an intent; --lang: go|python|ts|rust|java|cpp, others need --no-template", `{"name","path","rev","change_id"}`},
	...
	{"runko", "change land [--change <id>] [--repo . | -w <ws>] [--no-sync] [--json]", "land a mergeable change (default: HEAD's Change-Id; unique prefix OK) - on requires_revalidation it syncs, re-pushes, waits for checks, retries - needs a live runkod", "land.Outcome"},
	...
	{"runko", "change amend [--dir . | -w <ws>] [--allow-large] [-m <msg>] [--json]", "fold the working tree into HEAD's existing Change (keeps its Change-Id); plain git: `git commit --amend` with identity fallback; jj colocated: `jj squash` folds `@` into the change below. Refuses large/executable untracked files as suspected build artifacts unless --allow-large", `{"change_id"}`},
	...
	{"runko", "change ack-policy --change <id> [--by <who>] [--json]", "HUMANS ONLY (agents are refused): acknowledge an agent change's policy findings, completing the reserved agent-policy check - after a push warns about denylisted paths/size caps, TELL YOUR HUMAN this command, with the Change-Id", "checks.MergeRequirements"},
	...
	{"runko", "agents-md [--out AGENTS.md] [--json]", "regenerate this file AND the agent skill (.claude/skills/runko/SKILL.md) from the CLI's own command inventory", `{"path","skill_path","skill_paths"}`},
	...
	{"runko-ci", "checks --base <rev> [--head HEAD] [--post-land]", "resolve the affected closure's manifest-declared ci.checks for a CI executor; --post-land also includes every project's post_land-class checks unscoped", `{"run_everything","checks":[{"project","name","command"}]} (always JSON)`},
...
func writeTransparency(b *strings.Builder, verbose bool) {
	...
	b.WriteString("create/amend/push/requirements/land/automerge/describe/comment/comments/\n")
	b.WriteString("resolve/request-review, `workspace` snapshot/watch/branch/sync, and\n")
	b.WriteString("`agent hooks`. For `land`/`automerge`, `--change` defaults to HEAD's\n")
	b.WriteString("Change-Id inside a checkout (`-w <ws>` works); pass `--change` (or a\n")
	b.WriteString("unique prefix) for the checkout-free form. The pure server-side verbs\n")
	b.WriteString("(`approve`, `abandon`, `list`) still key off the Change-Id and never\n")
	b.WriteString("touch a checkout (`abandon` stays deliberately explicit), while\n")
	b.WriteString("`project create`, `doctor` and `agents-md` reach one only through\n")
	b.WriteString("`--repo` - for those,\n")
