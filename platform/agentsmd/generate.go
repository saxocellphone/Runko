...
var Commands = []Command{
	...
	{"runko", "ci init [--images] [--force] [--json]", "scaffold the generic CI/CD GitHub Actions workflows into .github/workflows/ (§14.9.1): a local file-writer copying templates/ci/*.yml (which download runko-ci and read every project/check/image/registry fact from the tree); --images adds the CD workflow. Then wire ci.checks + `github connect` + the RUNKO_URL/RUNKO_CI_TOKEN secrets", "ciInitResult"},
...
func writeTransparency(b *strings.Builder, verbose bool) {
	...
	b.WriteString("WHERE is an implementation detail (§12.7). The workspace NAME is the\n")
