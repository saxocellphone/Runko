...
| `runko agent hooks` | the ready-to-paste harness hooks JSON snippet on stdout (prerequisites on stderr); wires every post-tool-use call to `runko agent event --from-hook \|\| true` - local only |
