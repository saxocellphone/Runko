...
| `runko project create` | `{"name", "path", "rev"}`. `--lang` selects the built-in template language (`go` default, `python`, `ts`, `rust`, `java`, `cpp` — the §10.4 Bazel-maturity set); any other well-formed language needs `--no-template` (`unsupported_language` otherwise), which scaffolds only PROJECT.yaml + README and records the language verbatim |
