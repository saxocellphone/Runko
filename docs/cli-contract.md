...
| `runko workspace attach` | `WorkspaceInfo` + `Dir` - needs a live runkod. §12.7 placement/neutrality/auto-gc as on create (branch attaches default to `<workspace>@<branch>` under the managed home; attach never recycles - restore-from-snapshot is already the cheap path). Human mode ends with the §12.6 streaming next-commands; never in `--json` |
