...
| `runko workspace create` | `WorkspaceInfo` (`ID`, `Owner`, `BaseRevision`, `ProjectAffinity`, `WriteAllowlist`, `SnapshotRef`, `Status`, `SparsePatterns`, `RepoPath`, `TrunkRef`) - needs a live runkod, see §12.3/§28.3 stage 12b. Human mode ends with the §12.6 streaming next-commands (`workspace watch`, `agent hooks --install`); never in `--json` |
| `runko workspace list` | `[]WorkspaceInfo` - needs a live runkod |
| `runko workspace attach` | `WorkspaceInfo` - needs a live runkod. Human mode ends with the §12.6 streaming next-commands; never in `--json` |
