...
**Web frontend transport (draft, not yet decided):** a `.proto` schema for the web frontend ↔ `runkod` surface lives at `proto/runko/v1/` (started 2026-07-07, per direction that frontend/backend communication use gRPC), covering the same concepts as this section's REST/MCP surface (projects, changes, merge requirements, workspaces, search) - see `proto/README.md` for the full rationale, including the recommendation to serve it via Connect (gRPC + gRPC-Web + JSON from one Go server, no separate proxy) rather than raw grpc-go + Envoy. This is schema only - unimplemented, uncompiled (no `protoc`/`buf` in this sandbox), and NOT a replacement for the CLI/MCP's existing REST contract. Promote `proto/README.md`'s open questions into a real decision here before stage 13 wires a server against it.
