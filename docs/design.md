...
| Field | Value |
|-------|--------|
| **Status** | **Retired — frozen historical record** (was the living spec 2026-07-06 → 2026-07-16) |
| **Authors** | — |
| **Created** | 2026-07-06 |
| **Last updated** | 2026-07-16 (retirement; content frozen) |
| **Audience** | Engineering, product, design partners |

> **RETIRED 2026-07-16.** This document is no longer the living spec.
> Each project's `README.md` is now its spec surface — what it owns, its
> decided constraints, and a dated **Decisions** section that replaces
> this document's §25 changelog (see [`docs/README.md`](README.md) for
> the model). This file stays at this path, unedited, so the `§`
> citations throughout package headers, commit messages, and docs keep
> resolving as *historical rationale*. Do not add sections, revise
> decided content, or cite it for new work; §25's table is closed.
...
### 6.10 Org initiation: the golden path (CLI-first)

§6.9's bar, one step earlier: **a brand-new org's creator gets from nothing to a landed first Change guided only by command output.** Dogfooding found the opposite (2026-07-16): first contact required the web UI (no CLI signup), `org create` ended by telling the creator to re-type their password into a second `auth login`, the fresh org's trunk was unborn — `workspace create` dead-ended on `trunk_unborn`, and under default-deny **nothing could ever land** (no checks and no owners anywhere means no policy resolves) — and `runko doctor --install-hook` / `runko agents-md` were manual steps nothing announced. Decisions:

- **Genesis commit.** Org creation (the `org create` API and create-mode signup alike) seeds the new repo's trunk *before the org is announced*: a minimal root `PROJECT.yaml` (project `repo` — every path resolves an owning project, so `workspace create --project repo` works immediately), `OWNERS` naming the creator (§7.3 inheritance covers every path; the owner gate is never silently empty and default-deny resolves a policy from day one), the generated `AGENTS.md` and its loadable-skill twin at `.claude/skills/runko/SKILL.md` (§8.8 — same generator, frontmatter on top, so skill-loading harnesses pull the workflow into context at the moment of a change), and `CONTRIBUTING.md` carrying §6.9's three commands. Written directly by the server — genesis precedes the closed trunk the way `git init`'s unborn branch precedes history — and ordinary tree content thereafter (tree-as-truth, §10.3): orgs evolve or delete it through ordinary Changes. A born trunk is never rewritten (re-assembly after a crashed create is a no-op); the boot reload path and anonymous deploy-token creation (no one to seed OWNERS with) keep the bare behavior.
- **Uploader consent (Gerrit's model, §13.5 owner gate).** A *human* author who is themselves a required owner satisfies that requirement by having pushed the change's current head: the push is the consent the gate exists to collect, while self-approval stays denied as an action. Without this, genesis OWNERS would deadlock every solo org — a path whose only owner is the author was permanently unlandable. Agent authorship never self-satisfies: §8.7's "no approving at all" includes an agent's own push, so agent changes always keep a non-author human in the loop.
- **Signup is a CLI verb.** `runko auth signup --runkod-url <host> --name <you> --org <o> --create|--join` wraps `POST /api/signup` and stores the credential already pointed at the org's mount — signup IS login; first contact never requires the web UI. Server refusal shapes (§15.1's signup gates) relay verbatim per §6.5.
- **`org create` finishes signed in.** The hub clones the creating account's credential into the new org (per-org accounts, §15.1), so the CLI rebinds the stored login to the new org — whoami-verified before saving, `--no-switch` opts out, the pre-login `--token` scripting form is untouched — and prints the real next step instead of a second login command.
- **Materialized checkouts arrive wired.** `workspace create`/`attach` installs both client hooks into the shared clone (§6.9's verb nudge and the Change-Id commit-msg hook), never overwriting an existing hook — Gerrit's own commit-msg hook mints Change-Ids and must win; explicit `runko doctor --install-hook` keeps refresh-over-ours semantics. `workspace create` defaults `--by` to the stored login and prints the three-command loop next to the §12.6 streaming guidance. `doctor` and `agents-md` remain the diagnostic/regenerate verbs — neither is a required onboarding step.

**Bar:** `auth signup --create` → `workspace create --project repo` → edit → `change create` / `change push` / `change land`, with every command's output naming the next — zero wiki, zero web UI, zero second password prompt.
...
We provide **reference prompts / skill files** generated per monorepo, in two forms from one generator (so they cannot drift): `AGENTS.md` at the repo root — the ambient surface — and the identical body as a *loadable agent skill* at `.claude/skills/runko/SKILL.md`, frontmatter on top, so skill-loading harnesses (Claude Code and compatible) pull the workflow into context at the moment of a change instead of hoping the agent read AGENTS.md. Org genesis seeds the pair (§6.10); `runko agents-md` regenerates both. This doubles as stage 11's (§28.3) done-when bar, teaching the CLI as the primary agent interface:
...
### 12.7 Materialization lifecycle (decided 2026-07-15)

§12.3 decided the *shape* of local materialization — one blobless clone paying the object store once, N worktrees, each worktree one workspace — and stopped there. Nothing decided where those directories live, what tracks them, when they die, or what a new workspace may reuse from a dead one. Dogfooding made the gap measurable (migration finding #49): eighteen task materializations sitting **inside the developing checkout's own root** — polluting `git status` and every tool that walks the tree (gazelle destructively "fixed" BUILD files it reached through the nested trees) — ~2.4 GB of working trees whose blobless object stores total ~33 MB, three coexisting ad-hoc layouts, and 26 of 31 workspaces server-side **closed** (auto-closed correctly, finding #42) while every one of their materializations survived on disk. The product line is "a better worktree"; the dogfood floor was worse than plain `git worktree`. Four decisions close the gap, all CLI glue over `git worktree` — nothing here touches the §12.1 substrate stance.

**Placement — a managed home, no cwd-relative defaults.** Materializations default into one managed layout:

```text
$RUNKO_WORKSPACE_HOME/                    # default: ~/runko-ws
  <host>/<org>/<repo>/.store/             # THE shared blobless clone (--no-checkout; one per remote monorepo)
  <host>/<org>/<repo>/<workspace>/        # one worktree per workspace
  <host>/<org>/<repo>/<workspace>@<br>/   # parallel-branch attaches (§12.2) get sibling dirs
```

`--dir`/`--clone-dir` stay as explicit overrides (evals, tests, deliberate layouts), but the cwd-relative defaults (`mono`, worktree named into cwd) are abolished: run from the wrong directory, they materialized workspaces *into whatever checkout the caller stood in* — which is exactly how the in-tree sprawl happened. Explicit or defaulted, **materializing inside another git working tree is refused** (`workspace_nested_checkout`, suggestion: drop `--dir` and take the managed home; `--force-nested` for the rare deliberate case). `runko workspace path [<name>]` prints a materialization's directory, so "go to my workspace" is `` cd `runko workspace path fix-x` ``.

**Stores are credential-neutral.** A store's `origin` URL never embeds a credential; auth is injected per invocation from the *invoking* principal's credential store (git credential-helper shim; same resolution order as every verb: flags > env > stored login). This is the decision that makes sharing possible at all: the v1 glue baked the creating principal's token into the shared clone's remote URL (`composeRemoteURL` sets userinfo), so a second principal pushing through that store misattributed — and the only correct agent flow became a fresh clone per task, sharding the object store by *credential* when objects are principal-independent, content-addressed bytes. Isolation lives in the credential store (per-task agent principals, `XDG_CONFIG_HOME`), never in object storage. Recorded caveat, not solved now: restricted-visibility projects (§15.2) make the fetchable object *set* principal-dependent; an org enabling them shards stores per principal (`.store@<principal>`). The shared default stands because default visibility is org-wide — the same positioning that keeps Josh optional (§12.3 Phase B).

**A machine-local registry, rebuildable.** `$XDG_STATE_HOME/runko/materializations.json` records one row per worktree — workspace id, branch, path, store, remote, created/last-used. §10.3's stance applied locally: the registry is a **cache, never truth** — truth is the worktrees themselves, each already carrying worktree-scoped `runko.workspace`/`runko.branch` config — and `runko workspace gc --scan <store>` rebuilds rows by walking `git worktree list` + worktree config. `--scan` is also the adoption path for every pre-§12.7 layout: point it at a legacy clone and its worktrees enter lifecycle management. `runko workspace list` grows local columns when rows exist (path, dirty/synced, reclaimable). The server never learns local paths: machine state stays on the machine — the server-side registry row stays metadata-only per §12.2's invariant, and there is no ingest analogous to §12.6.1 because no cross-machine consumer exists.

**Reclamation — `runko workspace gc`, safe by construction.** The predicate is the durability contract cashed in: §12.2 promised snapshot refs survive laptop loss, so a synced materialization is *disposable by definition*. Reclaimable ⇔ the server says the workspace is closed (or gone) ∧ the working tree is clean ∧ HEAD is reachable from the last-pushed snapshot ref or trunk. Anything else — dirty, unpushed, server unreachable — is skipped with the reason printed (fail-closed, the funnel's posture). `gc` plans by default; `--apply` removes the worktree dir, runs `git worktree prune` on the store, drops the row. `--idle <dur>` extends the sweep to open-but-clean-and-synced workspaces (their durable state is server-side; re-attach recreates the directory in seconds). The store itself is never deleted while worktrees reference it, and its object maintenance is `git maintenance`'s job, not ours (§12.1's right-hand-column test: lifecycle *policy* is ours, storage *mechanics* are Git's). Nobody has to remember a chore verb: `workspace create`/`attach` first run a bounded auto-gc over unambiguously reclaimable rows (git's own auto-gc precedent), so steady-state disk is bounded by *open* workspaces; `runko doctor` reports reclaimable count and bytes; and the land path, which already knows when it just auto-closed a single-use workspace, says so in its output.

**Reuse — recycle worktrees; the cache is the expensive part.** The costly bytes of a materialization are not the checkout but the *ignored* state built on top — dependency trees and build caches (`web/node_modules` alone is ~296 MB of every full-cone checkout in this repo). Delete-and-re-add throws that away once per task. `workspace create` therefore prefers **rebinding** a reclaimable worktree on the same store over adding a fresh one: reset the worktree branch to the new base, rewrite the sparse cone to the new affinity, restamp the worktree-scoped `runko.*` config, rename the directory, and `git clean -fd` **without `-x`** — untracked residue from the dead task is removed, ignored caches survive. The marginal cost of workspace N+1 drops from clone + dependency install to a checkout delta. Recycling never crosses stores (a fortiori never remotes or orgs) and consumes only worktrees that pass the same gc predicate — a dirty or unpushed worktree is never recycled.

**A jj colocated checkout is a first-class materialization mode (decided 2026-07-16).** `workspace create/attach --jj` materializes the workspace as a **standalone jj colocated checkout** instead of a worktree off the shared store — runko sets the surgical client (§21) up itself rather than leaving it bring-your-own: `jj git init --colocate` over a fresh clone, the §7.4 trailer template wired (Change-Ids derive from jj change ids), the cone mirrored via `jj sparse` (prefix patterns; the repo root's files are enumerated explicitly for cone-mode parity), the working copy parked on the base/snapshot tip, and the `runko.*` binding in plain config scope — plus the same credential-neutral remote and identity hooks as the store. Standalone and FULL, by jj's own constraints: jj refuses to colocate inside a git worktree, and its backend cannot lazy-fetch promisor objects, so neither the shared store nor a blobless clone is available to this mode — §12.3's object-store dedup is the price of the jj working-copy model, paid only by those who opt in. Three consequences are encoded rather than left to be rediscovered: a `ws/<id>/<branch>` anchor branch makes an attach's snapshot tip resolvable at all (jj imports branches, never `refs/workspaces/*`); `workspace snapshot` routes through the out-of-band watch mechanics (a commit on the checked-out line would rewrite history behind jj's back), checked *before* the worktree-config lookup because `git config --worktree` silently degrades to `--local` without `extensions.worktreeConfig`; and `workspace branch` refuses with the recipe — a parallel line is a second standalone checkout (`attach --jj --branch`), while stacked work needs no branch at all. Registry rows for `--jj` checkouts carry no store and are never swept into worktree recycling.

**What this is not.** No background daemon or reaper (gc runs at verb time and create time), no new server state, no reopened VFS or streaming questions. If a future lifecycle need doesn't land in §12.1's right-hand column, it belongs in upstream Git configuration — this section is that principle applied to the directories the §12.3 glue creates. Implementation rides dogfood hardening (§28.3 stage 15); the verb contracts land in `docs/cli-contract.md` when they ship (spec-before-code: this section is the spec).
...
**Deletion is create's dual, and just as low-ceremony (decided 2026-07-15; user direction, born from the proto/ dissolution needing an admin to hand-delete a manifest).** `DeleteProjectIntent {name}` is the whole request. The plan (platform/project, pure): delete every file under the project's path AND surgically strip the project's name from every other manifest's `dependencies:`/`consumes:` lists — line-based edits, never a YAML re-marshal (manifests are comment-heavy; a round-trip would destroy them) — so no dangling edge can survive the deletion. Refused for the root project and unknown names. Preview is first-class, exactly like create. Apply is **server-authored**: the daemon commits the plan and registers an ordinary open Change (the `createProjectCore` mechanics — change ref, affected computation, webhooks), which then lands through the normal gates; deleting a project is reviewable and revertable like any other change, which is what makes subtree deletion acceptable UX. The CLI verb (`runko project delete`) calls the server rather than writing locally — deletion needs the trunk-tip index for edge-stripping, and a sparse local worktree may not even hold the files. **Agents cannot delete projects** (hard v1 refusal at the intent surface, the §8.7 posture; a human clicks the project page's delete button or types the verb).
...
1. **In-boundary contracts.** A project that serves an API owns its contract inside its own boundary. The §7.2 `rpc` capability gains its real shape: `capability_config.rpc: {path: <dir>}` (default `proto`) — proto sources at `<project>/<dir>/`, generated Go committed at `<project>/<dir>/gen/` (the `internal/dbgen` convention), each contract dir carrying its own `buf.gen.yaml`. The `http` capability gains the REST dual: `capability_config.http: {openapi: <file>}` (default `openapi.yaml`) — a project declaring a REST surface MUST carry an OpenAPI document in-boundary, and receive refuses a push that declares `http` without the document (or deletes it out from under the declaration). The standalone `proto/` project **no longer exists** (migrated 2026-07-15; it was briefly transitional with `rpc: {path: .}`): a contract has to live inside the project that serves it, so `runko/v1` moved into `runkod/proto/` beside `mailer/v1` — one codegen home, one owner — and web's edge flipped to `consumes: [runkod]`. Proto packages and Connect service paths were untouched (wire-compatible); only Go import paths moved.
...
3. **The API surface is decided at creation — and only serving types get one.** `runko project create` for a `service` requires `--api grpc|rest|none` (one enum flag — a product action, not hand-written YAML; the §2.3 layering holds): `grpc` scaffolds the `rpc` capability plus a v1 proto stub in-boundary; `rest` scaffolds the `http` capability plus a minimal OpenAPI 3.1 document; `none` is the explicit answer recorded by absence. An `app` may opt in (`--api` optional, same scaffolds). `library`/`job`/`other` cannot declare an API surface at creation (refined 2026-07-15, user direction): a library is consumed through build dependencies and a job has no callers — `grpc`/`rest` on those types is a structured `invalid_combination`, and the web wizard doesn't offer the choice at all. A job that grows a real serving surface later is a type change first.
...
Deferred, recorded: REST consumption without a generated client is invisible to the import scan (the mailer's old shape) — for REST the mandate is the artifact plus the honest `consumes:` edge by convention; TS/web consumption scanning remains open now that the `proto/` migration is done (web's `consumes: [runkod]` edge runs web-check exactly on `runkod/proto/**` changes; the import-scan detector still only reads Go); a Bazel-style `visibility:`/export allowlist on contracts is L3, on demand.

**Schema artifacts are the third contract shape; tests are encapsulated too (decided 2026-07-14).** `docs/spec/**` and `docs/cli-contract.md` are contracts that happen not to be proto — and the pre-§13.3.1 way of gating them broke encapsulation from the other side: the docs project's `contracts-test` hardcoded ANOTHER project's bazel targets (`//platform/agentsmd/... //platform/checks/... //platform/mcp/...`), going stale on any platform reorganization and reporting platform failures under docs' name. Decided: the `schemas` capability — `capability_config.schemas: {paths: [<project-relative>...]}` — declares a schema-artifact contract surface exactly like `rpc`'s path dir and `http`'s document; those paths (plus the manifest, fail closed) become the project's ContractPaths. The docs project declares `schemas: {paths: [spec, cli-contract.md]}`, **platform declares `consumes: [docs]`** (its three suites carry `data = ["//docs:contracts"]` — the true edge, visible in the build graph), and **`contracts-test` is deleted**: a spec change now reaches `platform-test` through the ordinary consumes closure, under platform's own name, with targets platform owns. The §14.5.7 obligation ("every prose exception names what still gates it") is answered by the edge instead of a foreign-target check; `platform-race` is `run_when: direct` and correctly skips schema edits. General rule extracted: **a check command never names another project's targets — if your tests consume my files, you declare the edge and your own check rides the closure.**
...
runko self-update           # replace this binary with the rolling cli-latest GitHub release build (checksum-verified)
...
| 2026-07-15 | **API surfaces are for serving types only (§13.3.1 leg ③ refined; user direction "grpc/rest should be for services/apps only - job/library/other shouldn't have that option")**: `--api grpc\|rest` on a `library`/`job`/`other` project is a structured `invalid_combination` (a library is consumed through build deps, a job has no callers; a job that grows a serving surface is a type change first); `app` keeps the optional opt-in, `service` keeps the mandate. Enforced in `project.Validate` (every intent path: CLI, Connect wizard, MCP), and the web wizard hides the picker entirely for ineligible types, clearing a previously-picked answer on type switch |
| 2026-07-15 | **The standalone proto/ project is gone (§13.3.1 leg ① completed; user direction "the proto/ folder shouldn't exist, because it has to be part of another project")**: `runko/v1` moved into `runkod/proto/` beside `mailer/v1` — the serving project owns its whole contract surface, one buf.gen.yaml home — with go_package/import paths updated (runkod is the only Go consumer; self-consumption now, no edge needed), web's TS generation re-pointed at `runkod/proto` filtered to `runko/v1` (descriptor names and output paths unchanged — buf module-root-relative naming is preserved, so zero TS import churn), and proto packages/Connect service routes untouched (wire-compatible). The ops manifests complete it: proto/PROJECT.yaml deleted, runkod's dependency on proto dropped, web flips to `consumes: [runkod]` |
| 2026-07-15 | **Project deletion is a product verb (§13.1; user direction "implement a project delete cli and UI feature" - the proto/ dissolution exposed that retiring a project meant an admin hand-deleting a manifest)**: `DeleteProjectIntent {name}` → preview → server-authored Change (createProjectCore mechanics: daemon commits the plan, ordinary open Change, normal gates - reviewable, revertable). The plan deletes the project subtree AND line-surgically strips the name from every other manifest's dependencies/consumes (comments preserved; no dangling edges). Root project and unknown names refused; AGENTS REFUSED at the intent surface (§8.7 posture). Surfaces: `runko project delete` (server-calling - edge-stripping needs the trunk index, sparse worktrees may lack the files), Connect Preview/DeleteProject RPCs, a confirm-gated Delete button on the web project page |
| 2026-07-14 | **Schema artifacts are the third contract shape; test encapsulation (§13.3.1 extended; user direction "it concerns me that the docs project is running bazel test of another project - we should consider encapsulation for tests as well")**: the `schemas` capability (`capability_config.schemas: {paths: [...]}`) declares a non-proto contract surface, joining rpc/http; docs declares `schemas: {paths: [spec, cli-contract.md]}`, platform declares `consumes: [docs]` (its suites carry `data = ["//docs:contracts"]` - the true, graph-visible edge), and docs' `contracts-test` is DELETED - the pre-§13.3.1 shape hardcoded platform's bazel targets in docs' check command (stale on reorganization, failures misattributed, double-run on mixed changes); a spec change now reaches platform-test through the ordinary consumes closure under platform's own name (platform-race is run_when: direct and correctly skips schema edits). General rule: a check command never names another project's targets - if your tests consume my files, declare the edge and let your own check ride the closure |
| 2026-07-15 | **Materialization lifecycle decided (§12.7 new; migration finding #49; user direction "a selling point of Runko is that we have a better worktree, and having so many worktrees cluttering the file system is not helping — design a system so that we can improve on the current worktree utilization")**: §12.3 decided the shape (one blobless store, N worktrees) but nothing decided placement, tracking, death, or reuse — dogfooding left 18 task materializations inside the developing checkout's root (~2.4 GB vs ~33 MB of object stores; 26 of 31 workspaces closed with every materialization still on disk; gazelle stripped BUILD deps through the nested trees). Four decisions, all CLI glue over `git worktree`: (1) managed home `$RUNKO_WORKSPACE_HOME` (default `~/runko-ws`, `<host>/<org>/<repo>/{.store,<workspace>[@<branch>]}`), cwd-relative defaults abolished, materializing inside another working tree refused (`workspace_nested_checkout`; `--force-nested` escape), `workspace path` for scripting; (2) credential-neutral stores — auth injected per invocation via credential helper, never baked into the remote URL (the baked-token flaw forced clone-per-task and sharded content-addressed objects by credential; isolation lives in the credential store, never object storage); §15.2 restricted-visibility orgs shard `.store@<principal>`; (3) machine-local REBUILDABLE registry (`$XDG_STATE_HOME/runko/materializations.json`; truth = worktree-scoped runko.* config; `gc --scan` rebuilds and ADOPTS legacy layouts; server never learns local paths); (4) `workspace gc` — reclaimable ⇔ server-closed ∧ clean ∧ HEAD reachable from pushed snapshot/trunk, fail-closed skips with printed reasons, plan-by-default/`--apply`, `--idle` for open-but-synced, bounded auto-gc at create/attach, doctor reports reclaimable bytes — plus worktree RECYCLING: create rebinds a reclaimable worktree (reset base, rewrite cone, restamp config, rename, `git clean -fd` without `-x`) so ignored caches (node_modules ≈ 296 MB per full-cone checkout) survive and workspace N+1 costs a checkout delta, not clone + dependency install |

| 2026-07-15 | **Land-stack verb (§13.5's bottom-up landing loop becomes one call; user direction "implement a land all feature, where it will land all changes from the current change to the bottom of the stack")**: `ChangeService.LandStack` / `POST /api/changes/{key}/land-stack` lands the requested Change's open ancestor chain bottom-up, then the Change itself — each member through the exact same landChangeCore as a single land (same per-principal merge gate, attribution, webhooks, automerge kick, workspace bookkeeping): the Gerrit "submit including parents" analog, and the server-side form of the land-parent-wait-land-child loop every client hand-rolled. The sweep stops at the first member that cannot land and reports that member's own blockers/conflicts/revalidation; landed members are DURABLE partial progress, so per-member failures are encoded in the response report, never returned as transport errors that would bury them (hence the REST sibling's one deliberate divergence from one-clierr-per-outcome: a stopped sweep still answers 200 with the report). Dependents above the requested Change stay open; there is no force variant (the §13.5 override stays a single-change, eyes-on verb); the chain walk follows stackForChange's aliveness rule — a landed/abandoned/unknown base ends it, leaving refuseUnlandedParent the authority on whether the bottom member may land. Web: the change page grows "Land all (N)" whenever open ancestors exist (enabled even while the change itself is parent-blocked — driving the chain is the point) plus sweep-report banners; /demo's fake transport mirrors the sweep so the playground exercises both the landed-through and stopped-mid-chain paths. Pinned by chain-through-middle/stop-and-resume/abandoned-refusal core tests |
| 2026-07-16 | **Org git URLs are org-named (§7.1 polish; user direction "when you clone from remote, the folder name is always repo/ - it should be the org name")**: every org repo stays on disk at `<orgs-dir>/<org>/repo.git`, but the ADVERTISED clone URL becomes `/o/<org>/<org>.git` (orgInfoFor git_url, the workspace API RepoPath, the agent-mint hint) - a plain `git clone` of it names the checkout folder after the org instead of the meaningless "repo". Served by a path-rewrite alias onto the on-disk mount (git http-backend keys PATH_INFO to the directory basename); the historical `/o/<org>/repo.git` path stays served forever, so every pre-alias remote and CI config keeps working (pinned in the orgs e2e). Root-mounted default servers already advertise their repo-dir basename - which IS the default org name by construction - unchanged |
| 2026-07-16 | **Org initiation golden path (§6.10 new; user direction "the org initiation is very painful from the CLI perspective... there's no OWNERS file by default which is an antipattern")**: five decisions closing the fresh-org dead ends. (1) GENESIS COMMIT: org creation seeds trunk before the org is announced - minimal root PROJECT.yaml (project `repo`; `workspace create --project repo` works immediately, no more trunk_unborn), OWNERS naming the creator (§7.3 inheritance; the owner gate is never silently empty and default-deny resolves a policy from day one - previously a fresh org could land NOTHING: no checks, no owners, no policy), generated AGENTS.md, CONTRIBUTING.md with §6.9's three commands; written directly (genesis precedes the closed trunk like git init's unborn branch), ordinary tree content thereafter; born trunks never rewritten, boot-reload/anonymous creation keep bare behavior. (2) UPLOADER CONSENT (Gerrit's model, §13.5 owner gate): a human author who is a required owner satisfies that requirement by having pushed the head - self-approval stays denied as an action; without this genesis OWNERS deadlocks every solo org (sole-owner paths were permanently unlandable); agent authorship never self-satisfies (§8.7). (3) `runko auth signup --create\|--join`: signup IS login - wraps POST /api/signup, stores the credential pointed at the org mount, first contact never needs the web UI. (4) `org create` rebinds the stored login to the new org (the hub already cloned the credential there - per-org accounts; whoami-verified, `--no-switch` opts out) instead of demanding a second password entry. (5) Materialized checkouts arrive wired: workspace create/attach installs BOTH client hooks (verb nudge + Change-Id, never overwriting an existing commit-msg hook - Gerrit's own must win), `--by` defaults to the stored login, `workspace create` prints the three-command loop; doctor/agents-md demoted from required steps to diagnostic/regenerate verbs. Bar: signup → workspace create → change create/push/land, every output naming the next - zero wiki, zero web UI, zero second password prompt |
| 2026-07-16 | **runko sets up jj itself (§12.7; §21's positioning unchanged; user direction "the runko cli should setup jj as well" - the surgical client was bring-your-own: a colocated checkout took a manual clone + doctor + two config binds)**: `workspace create/attach --jj` materializes a STANDALONE jj colocated checkout - `jj git init --colocate` over a fresh FULL clone (jj refuses to colocate inside a git worktree and cannot lazy-fetch promisor blobs, so neither the shared store nor blob:none apply; `--clone-dir` refused with `jj_no_shared_store`), the §7.4 trailer template wired, the cone mirrored via `jj sparse` with the repo root's files enumerated (cone-mode parity), @ parked on the base/snapshot tip via a `ws/<id>/<branch>` anchor branch (jj imports branches, never refs/workspaces/*), runko.* bound in plain config scope, and the same credential-neutral remote + identity hooks as the store. `workspace snapshot` in a bound jj checkout routes OUT-OF-BAND (watch.go mechanics - a commit on the checked-out line would rewrite history behind jj's back), checked before the worktree lookup because `git config --worktree` silently degrades to --local without extensions.worktreeConfig; `workspace branch` refuses with the recipe (a parallel line is a second standalone checkout: `attach --jj --branch`). Structured suggestions now speak the checkout's dialect (§6.5): nothing_to_commit rewords via `jj describe` (the plain-git amend it used to offer is the exact rewrite the push guard refuses), outside_sparse_cone widens via `jj sparse set --add`, already_on_trunk offers `jj commit`, the empty-state tips name `jj git init --colocate` beside `git init`, and the workspace-binding tips name --jj. Pinned by create/attach round-trip, out-of-band snapshot, and branch-refusal tests |
| 2026-07-16 | **CLI self-update (§17.1; user direction "build an update feature in the cli so the runko cli can update itself - use the GitHub release as source of truth")**: `runko self-update` (alias `update`) replaces the running binary with the rolling `cli-latest` GitHub release build - release-images' `cli-release` job (per-platform assets + checksums.txt, republished on every CLI-affecting landing) was already the distribution channel; this makes the installed binary a client of it. Identity is CONTENT, not a version string: up to date ⇔ the binary's sha256 equals the release's checksums.txt entry for this GOOS/GOARCH, so a from-source build reads as stale and converges on the release (the release being the source of truth, convergence is the feature, not a downgrade - `go install .../cli/runko@latest` remains the from-source alternative every structured failure suggests). The download is verified against the published checksum BEFORE any swap; the swap is atomic (temp file beside the binary + rename, so no torn binary exists at any instant; symlinked installs resolve to their target; Windows renames the running image aside to `.old` since a live .exe can be renamed but not overwritten). `--check` reports without installing (the answer is output, not exit code); `--repo` retargets a fork's release. GitHub's API is consulted anonymously over stdlib HTTP - zero new deps (§22.2 lean-deps) |
| 2026-07-16 | **Initialization installs the agent skill (§6.10 genesis + §8.8; user direction "when we initialize runko we also install the agent skill as well")**: AGENTS.md is ambient - read by whoever happens to look - while skill-loading harnesses (Claude Code and compatible) pull a skill into context at the moment of a change, keyed on its frontmatter description. The generated teaching now ships in BOTH forms from ONE generator (`agentsmd.GenerateSkill` = frontmatter + `Generate()`'s body verbatim, `agentsmd.SkillPath` = `.claude/skills/runko/SKILL.md`, the harness-discoverable project location): org genesis seeds the pair alongside OWNERS/CONTRIBUTING.md, and `runko agents-md` regenerates both (`--out` still moves only AGENTS.md; the skill's path is part of how harnesses find it and stays fixed; JSON gains `skill_path`). One generator behind both surfaces is the drift story - a CLI change regenerates two files or none. Dogfood: this repo's own regenerated pair landed with the verb change, which also caught AGENTS.md up with inventory rows the §6.10/§12.7 series had added without rerunning the verb |
| 2026-07-16 | **This document is RETIRED (user direction "we reached a point where keeping a centralized spec isn't helpful anymore - retire design.md as the spec; keep a README.md per project")**: the spec served its purpose - §28.3's DAG is implemented and the platform develops on itself - and a centralized document now fights the encapsulation the platform itself enforces (projects own their checks per §14.9.1; now they own their docs). Each project's README.md becomes its spec surface: what it owns, decided constraints, contract surfaces, checks, and a dated Decisions section replacing THIS table (repo-wide decisions go in the root README.md; docs/README.md documents the model). This file is frozen in place - Status flipped, banner added, not otherwise edited - so the § citations throughout package headers, commit messages, and history keep resolving as rationale. docs/spec/** and cli-contract.md remain live, load-bearing contract artifacts (the docs project's declared schemas surface); implementation-log.md and migration-findings.md continue as the histories. This row closes the table |
