...
**Web frontend transport (client side decided 2026-07-07; server half pending):** a `.proto` schema for the web frontend ↔ `runkod` surface lives at `proto/runko/v1/` (started 2026-07-07, per direction that frontend/backend communication use gRPC), covering the same concepts as this section's REST/MCP surface (projects, changes, merge requirements, workspaces, search) - see `proto/README.md` for the full rationale. Stage 13's frontend half (`web/`) now consumes this contract via **Connect-ES** (generated protobuf-es types committed at `web/src/gen`), confirming the Connect recommendation on the client side and wire-freezing the field numbers; `GetChangeStack` + `GetChangeDiff` were added for the stacked-diff view (§17.2). Until runkod mounts connect-go handlers (the remaining half of stage 13), the frontend runs on an in-memory fake transport with the same generated types. This surface is NOT a replacement for the CLI/MCP's existing REST contract.
...
| 13 | Minimal web (SSR + htmx) | 10, 11b, 11c, 12c | Wizard + change page + merge requirements + a land button wired to 11b's endpoint, gated by 11c's policy |
