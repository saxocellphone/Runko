...
function planProject(state: FakeState, intent: CreateProjectIntent | undefined) {
  ...
  // Bazel is the org default (2026-07-08): every template carries the
  // build capability, so the manifest and a generated BUILD.bazel mirror
  // what project.DefaultTemplates emits server-side.
  const manifest = [
    "schema: project/v1",
    `name: ${name}`,
    `type: ${type}`,
