...
2. **Secrets** (repo → Settings → Secrets): set `RUNKO_URL` to your org mount
   (e.g. `https://<host>/o/<org>`) and `RUNKO_CI_TOKEN` to a CI token for that
   org. GitHub provides `GITHUB_TOKEN` automatically.
3. **Wire the dispatch**: `runko github connect --repo <owner>/<name>` points
   your org's outbox at this GitHub repo, so a pushed Change fires
   `runko-change` and a land fires `runko-image-build`. The workflows fetch the
   change ref from the mirror (with a retry, since the dispatch can beat the
...
The executor structure is generic, but your check *commands* need a runner.
The template ships with **Go only** (required to run `runko-ci` itself) and two
commented blocks marked `RUNNER CONTRACT (ADJUST)`:
